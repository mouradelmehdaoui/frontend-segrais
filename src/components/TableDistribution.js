...
export default function TableDistribution({ data }) {
  ...
  return (
    <>
      <button className="btn btn-success mb-3" onClick={handleAdd}>Ajouter</button>
          ...
          {data.map(d => (
            <tr key={d._id}>
              ...
              <td>
                ...
                <button className="btn btn-danger btn-sm" onClick={() => dispatch(deleteDistribution(d._id))}>Supprimer</button>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      <DistributionForm show={showForm} onClose={() => setShowForm(false)} editData={editData} />
    </>
  );
}
